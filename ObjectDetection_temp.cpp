...
//	boost::split(strs, imgName, boost::is_any_of("g") );
	std::string line = "testigtest";
//	if( strs.empty() == true ) return -1;
	...
	//cvSetImageROI(&img, cvRect(x_left, y_left, x_wid, y_ht));
		...
		for(int x = 0; x < wid; x += stride) {
			for(int y = 0; y < ht; y += stride) {
				if( (x + 64) > wid || (y + 64) > ht)
					continue;
				// detect
				cv::Mat blockImg = boundedImg(cv::Range(y,y+64), cv::Range(x, x+64));
...
int ObjectDetection::InitTest(std::string modelName)
{
	...
	psvm->GetSupportVector(supVector);
        ...
        m_phog = new cv::HOGDescriptor(cv::Size(64, 64), cv::Size(16,16), cv::Size(8,8), cv::Size(8,8), 9, 1, -1,
                                                                cv::HOGDescriptor::L2Hys, 0.2, true, cv::HOGDescriptor::DEFAULT_NLEVELS);
